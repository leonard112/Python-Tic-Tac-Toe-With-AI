...
"""
AI used to help the computer make the best move possible.
If the AI in this module cannot find a good move, a move will be made using the computerTurnRandom() module.
PARAM: "o" String variable that represents player O.
PARAM: "board" the 2 dimensional array that stores the players' moves.

RETURN: 2 dimensional array that represents an updated version of the board.
"""
def computerTurnWithAI(o, board):
    ...
    for row in range(3): #for each row
        ...
        if blankCount > 0: #if the row is not full
            if oCount == 2 and rowScore < 4:
                rowScore = 4
                bestRow = row
            elif xCount == 2 and rowScore < 3:
                rowScore = 3
                bestRow = row
    ...
    for col in range(3): #for each column
        ...
        if blankCount > 0: #if the column is not full
            if oCount == 2 and colScore < 4:
                colScore = 4
                bestCol = col
            elif xCount == 2 and colScore < 3:
                colScore = 3
                bestCol = col
    ...
    if blankCount > 0: #if the back diagonal is not full
        if oCount == 2 and backDiagonalScore < 4:
            backDiagonalScore = 4
        elif xCount == 2 and backDiagonalScore < 3:
            backDiagonalScore = 3
    ...
    if blankCount > 0: #if the forward diagonal is not full
        if oCount == 2 and forwardDiagonalScore < 4:
            forwardDiagonalScore = 4
        elif xCount == 2 and forwardDiagonalScore < 3:
